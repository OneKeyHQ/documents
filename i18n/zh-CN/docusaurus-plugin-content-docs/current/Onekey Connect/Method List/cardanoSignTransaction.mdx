...
- `inputs` — _必需_ [CardanoInput](https://github.com/OneKeyHQ/connect/blob/onekey/src/js/types/networks/cardano.js#L59) 对象的`数组类型`
- `outputs` - _必需_ [CardanoOutput](https://github.com/OneKeyHQ/connect/blob/onekey/src/js/types/networks/cardano.js#L74) 对象的`数组类型`
- `fee` - _必需_ `String`
- `protocolMagic` - _必填_ `Integer` 764824073（用于主网），42（用于 Testnet）
- `networkId` - _必填_ `Integer` 1 代表 Mainnet，0 代表 Testnet
- `ttl` - _可选_ `String`
- `validityIntervalStart` - _可选_ `String`
- `certificates` - _可选_ [CardanoCertificate](https://github.com/OneKeyHQ/connect/blob/onekey/src/js/types/networks/cardano.js#L119) 对象的`数组类型`
- `withdrawals` - _可选_ [CardanoWithdrawal](https://github.com/OneKeyHQ/connect/blob/onekey/src/js/types/networks/cardano.js#L126) 对象的`数组类型`
- `metadata` - _可选_ `String`

### 关于质押池 Stake Pool 验证

OneKey 支持作为池所有者对质押池注册证书进行签名。
交易可能包含外部信息(比如池操作员提供的信息)，OneKey 无法验证它们是否来自外部，因此，如果我们允许使用密钥对交易进行签名，则可能会使用户从输入中损失不打算花费的钱。
