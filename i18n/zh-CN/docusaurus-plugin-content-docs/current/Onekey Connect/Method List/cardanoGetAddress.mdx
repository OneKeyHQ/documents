...
## Cardano：获取地址
在设备上显示由给定的 [BIP32-Ed25519](https://cardanolaunch.com/assets/Ed25519_BIP.pdf) 路径派生的请求地址，并将其返回给调用方。向用户显示所请求密钥的描述，并要求确认 OneKey 上导出。
...
* `addressType` - *必填*` CardanoAddressType`/`number`。您可以使用 `CARDANO.ADDRESS_TYPE` 对象或使用 `CardanoAddressType` 枚举。支持 「Base address」,「Pointer address」,「Enterprise address
」,「Byron」 和 「Reward address」 这些类型。
* `path` - *必需* `string | Array<number>` 的最小长度为 5。 [了解更多](./path)
