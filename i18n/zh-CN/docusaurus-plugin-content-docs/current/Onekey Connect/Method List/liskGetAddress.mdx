...
## Lisk：获取地址
在设备上显示由给定的BIP32路径派生的请求地址，并将其返回给调用者。向用户显示所请求密钥的描述，并要求确认从OneKey上的导出。
...
#### 处理按钮请求
从onekey-connect@6.0.4开始，就有可能处理UI.ADDRESS_VALIDATION事件，一旦在设备上显示地址该事件就会触发。您可以处理此事件并在应用程序内部显示自定义UI。
