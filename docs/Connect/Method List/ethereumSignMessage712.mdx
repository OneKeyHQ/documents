...
* `version` - *optional* `string` `V3` or `V4`, for `sign_typed_data` version, default is V3.
* `data` - *required* `object`, message data type. see example for more detail.
...
5. not support EIP712Domain primary type, your passed data's `primaryType` must not be `EIP712Domain`
```json
{
  "success": false,
  "payload": {
